...
import OrderHeighlights from "./Routes/OrderHighlights";
...
-
  ...
  const countOrdersByStatus = (status) => {
    if (status === 'Pending') {
      // Include both 'New' and 'Pending' orders in the count for Pending
      return orders.filter((order) => order.status === 'New' || order.status === 'Pending').length;
    }
    // Otherwise, count based on the provided status
    return orders.filter((order) => order.status === status).length;
  };
  
  const orderss = [
    { title: "New Order", status: 'New', count: countOrdersByStatus("New"), icon: "🆕" },
    { title: "Pending Order", status: 'Pending', count: countOrdersByStatus("Pending"), icon: "⏳" }, // This includes 'New' and 'Pending' orders
    { title: "Approved Order", status: 'Approved', count: countOrdersByStatus("Approved"), icon: "✅" },
    { title: "Canceled Order", status: 'Canceled', count: countOrdersByStatus("Canceled"), icon: "📦" },
  ...
  const pieChartData = [
    ...
    { name: 'Canceled', value: countOrdersByStatus('Canceled') },
...
<div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4  pt-4 pb-4  ">
            ...
            {orderss.map((order, index) => (
              <Link key={index}>
  <div
    className="bg-gray-800 cursor-pointer hover:bg-gray-500 text-white rounded-lg p-6 flex flex-col items-center justify-between"
  ...
  >
    ...
    <button className="bg-red-500 text-white rounded-full py-2 px-6">
      + {order.count}
        ...
        <div>
        <OrderHeighlights></OrderHeighlights>
       </div>
